                                             ...
                                             void *rbuf, size_t rcount, MPI_Datatype rdtype, MPI_Comm comm)
{
  ...
  if (!(local_rank & 1) || local_rank >= remaning_local << 1 || local_sub_group == GPU_ON_NODE)
  {
    ...
    for (int i = 0; i < local_sub_group; i++)
    {
      ...
      /*printf("rank %d peer %d send data %d recv data %d send pos %d recv pos %d\n", rank, peer, local_data_to_send, local_data_to_recv, (ptrdiff_t)rank * (ptrdiff_t)rcount, (ptrdiff_t)(node_offset + i) * (ptrdiff_t)rcount);
      fflush(stdout); */
    }
                                 ...
                                 void* rbuf, size_t rcount, MPI_Datatype rdtype, MPI_Comm comm)
{
  ...
  int remote, distance, send_block_location;
  ptrdiff_t rlb, rext;
...
int allgather_bine_block_by_block_hierarchical_local_global(const void *sbuf, size_t scount, MPI_Datatype sdtype,
                           void* rbuf, size_t rcount, MPI_Datatype rdtype, MPI_Comm comm){
  int line = -1, rank, size, steps, err = MPI_SUCCESS, remote;
...
int allgather_bine_block_by_block_hierarchical_global_local(const void *sbuf, size_t scount, MPI_Datatype sdtype,
                           void* rbuf, size_t rcount, MPI_Datatype rdtype, MPI_Comm comm){
  int line = -1, rank, size, steps, err = MPI_SUCCESS, remote;
  int node_size, node_rank, node_offset, local_rank;
  int num_reqs;
...
int allgather_bine_send_remap_hierarchical_local_global(const void *sbuf, size_t scount, MPI_Datatype sdtype,
                           void* rbuf, size_t rcount, MPI_Datatype rdtype, MPI_Comm comm)
{
  int line = -1, rank, size, steps, err = MPI_SUCCESS;
  int vrank, remote, vremote, send_block_location, distance;
  ...
  */
  ...
  for(int step = steps - 1; step >= 0; step--) {
    ...
    if(vrank < vremote){
      tmpsend = (char*)rbuf + (ptrdiff_t)send_block_location * GPU_ON_NODE * (ptrdiff_t)rcount * rext;
      tmprecv = (char*)rbuf + (ptrdiff_t)(send_block_location + distance) * GPU_ON_NODE * (ptrdiff_t)rcount * rext;
    } else {
      tmpsend = (char*)rbuf + (ptrdiff_t)send_block_location * GPU_ON_NODE * (ptrdiff_t)rcount * rext;
      tmprecv = (char*)rbuf + (ptrdiff_t)(send_block_location - distance) * GPU_ON_NODE * (ptrdiff_t)rcount * rext;
      send_block_location -= distance;
...
int allgather_bine_send_remap_hierarchical_global_local(const void *sbuf, size_t scount, MPI_Datatype sdtype,
                           void* rbuf, size_t rcount, MPI_Datatype rdtype, MPI_Comm comm)
{
  int line = -1, rank, size, steps, err = MPI_SUCCESS;
  int vrank, remote, vremote, send_block_location, distance;
...
int allgather_bine_2_blocks_hierarchical(const void *sbuf, size_t scount, MPI_Datatype sdtype,
                           void* rbuf, size_t rcount, MPI_Datatype rdtype, MPI_Comm comm)
{
  int line = -1, rank, size, steps, err = MPI_SUCCESS, remote;
...
int allgather_bine_permutation_hierarchical_local_global(const void *sbuf, size_t scount, MPI_Datatype sdtype,
                           void* rbuf, size_t rcount, MPI_Datatype rdtype, MPI_Comm comm){
  int line = -1, rank, size, steps, err = MPI_SUCCESS, remote, data_exchange;
...
int allgather_bine_permutation_hierarchical_global_local(const void *sbuf, size_t scount, MPI_Datatype sdtype,
                                            void *rbuf, size_t rcount, MPI_Datatype rdtype, MPI_Comm comm)
{
  int line = -1, rank, size, steps, err = MPI_SUCCESS, remote, data_exchange;
  int node_size, node_rank, node_offset, local_rank, num_reqs;
