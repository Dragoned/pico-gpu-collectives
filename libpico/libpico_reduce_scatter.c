                                                      ...
                                                      MPI_Datatype dtype, MPI_Op op, MPI_Comm comm)
{
  ...
  int peer, dist_mask, rem_data, seg_num;
  int send_index = 0, recv_index = 0;
  size_t send_size, recv_size, seg_size = 0;
  ptrdiff_t *disps = NULL;
  MPI_Request *send_req = NULL;
  MPI_Request *recv_req = NULL;
  int send_req_index, recv_req_index, node_offset;
  ...
  {
    seg_num = 0;
    for (i = 0; i < size - node_size; i += node_size)
    {
      seg_num = max(disps[i + node_size] - disps[i], seg_num);
    }
    seg_num = max(max(rcounts[size - 1] - disps[i], seg_num) * extent / bine_allreduce_segsize, 1);
    seg_size = bine_allreduce_segsize / extent;
  }
  else
  {
    seg_num = 1;
  ...
  for (size_t seg_ind = seg_size; seg_ind < send_size; seg_ind += seg_size)
  {
    ...
    for (i = 0; i < GPU_ON_NODE; i++)
    {
      ...
      err = MPI_Isend(sbuf + (disps[local_inverse * node_size] + seg_ind) * extent, seg_size, dtype, peer, 0, comm, &send_req[send_req_index]);
      if (err != MPI_SUCCESS)
        goto cleanup;
      ...
      /*printf("rank %d send %d reciv %d seg index %d seg size %d\n", rank, disps[local_inverse * node_size] + seg_ind, recv_index, seg_ind, seg_size);
      fflush(stdout);*/
    ...
    err = reduce_wrapper_grops_inoutsplit(recv_buff_head + reduce_recv_offset * extent, result_buff_head + reduce_offset * extent,
                                          sbuf + (disps[local_inverse * node_size] + reduce_offset) * extent, seg_size, GPU_ON_NODE - 1, dtype, op);
    if (err != MPI_SUCCESS)
      goto cleanup;
#else
...
{
  int size, rank, dtsize, err = MPI_SUCCESS;
  int local_rank, node_size, node_offset, node_rank;
  MPI_Comm_size(comm, &size);
  MPI_Comm_rank(comm, &rank);
  MPI_Type_size(dt, &dtsize);
  ...
  void *tmpbuf, *resbuf, *perm_buff[2];
  PICO_TAG_BEGIN("setup/alloc_temp_buff");
#ifdef PICO_MPI_CUDA_AWARE
  BINE_CUDA_CHECK(cudaMalloc(&tmpbuf, (local_rcount[local_rank] * GPU_ON_NODE + perm_buff_size * 2) * dtsize));
...
#else
  tmpbuf = malloc((local_rcount[local_rank] * GPU_ON_NODE + perm_buff_size * 2) * dtsize);
  ...
  perm_buff[0] = resbuf + (ptrdiff_t)local_rcount[local_rank] * (ptrdiff_t)dtsize;
  perm_buff[1] = perm_buff[0] + (ptrdiff_t)local_rcount[local_rank] * (ptrdiff_t)dtsize;
  if (NULL == displs || NULL == step_to_send || NULL == tmpbuf || NULL == resbuf || perm_buff_size <= 0)
  {
    err = MPI_ERR_NO_MEM;
    goto err_hndl;
  }
  ...
  for (int i = 0; i < GPU_ON_NODE; i++)
  {
    ...
    if (local_rcount[i] > 0)
    {
      err = MPI_Isend(perm_buff[buf_sel], local_rcount[i], dt, partner, 0, comm, &perm_send_req);
      if (MPI_SUCCESS != err)
      {
        goto err_hndl;
      }
  ...
  while (mask < node_size)
  {
    ...
    send_block_first = remap_rank(node_size, partner) & block_first_mask;
    send_block_last = send_block_first + inverse_mask - 1;
    send_block_first = send_block_first + res_first_node;
    send_block_last = send_block_last + res_first_node;
    int send_count = displs[send_block_last] - displs[send_block_first] + recvcounts[send_block_last];
    // Something similar for the block to recv.
    // I receive my block, but aligned to the power of two
    recv_block_first = remapped_rank & block_first_mask;
    recv_block_last = recv_block_first + inverse_mask - 1;
    recv_block_first = recv_block_first + res_first_node;
    recv_block_last = recv_block_last + res_first_node;
    int recv_count = displs[recv_block_last] - displs[recv_block_first] + recvcounts[recv_block_last];

    /*
    printf("recv_block %d, send_block %d, offset %d, rank %d\n", recv_count, send_count, displs[send_block_first], rank);
    fflush(stdout);
    */
                                                       ...
                                                       MPI_Datatype dt, MPI_Op op, MPI_Comm comm)
{
  ...
  PICO_TAG_BEGIN("setup/alloc_temp_buff");
#ifdef PICO_MPI_CUDA_AWARE
  BINE_CUDA_CHECK(cudaMalloc(&tmpbuf, (count / GPU_ON_NODE * (GPU_ON_NODE - 1)) * dtsize));
  BINE_CUDA_CHECK(cudaMalloc(&resbuf, (count / GPU_ON_NODE) * dtsize));
#else
  tmpbuf = malloc((count / GPU_ON_NODE * (GPU_ON_NODE - 1)) * dtsize);
  resbuf = malloc((count / GPU_ON_NODE) * dtsize);
#endif
  PICO_TAG_END("setup/alloc_temp_buff");

  PICO_TAG_BEGIN("setup/alloc_support_buf");
  int *inverse_remapping = (int *)malloc(node_size * sizeof(int));  
  MPI_Request *send_req = (MPI_Request *)malloc(node_size * (GPU_ON_NODE - 1) * sizeof(MPI_Request));
  MPI_Request *recv_req = (MPI_Request *)malloc(node_size * (GPU_ON_NODE - 1) * sizeof(MPI_Request));
  PICO_TAG_END("setup/alloc_support_buf");
  ...
  PICO_TAG_BEGIN("local_com/recv_wait");
  err = MPI_Waitall(recv_req_count, recv_req, MPI_STATUSES_IGNORE);
  PICO_TAG_END("local_com/recv_wait");

  PICO_TAG_BEGIN("local_com/kernel");
#ifdef PICO_MPI_CUDA_AWARE
  err = reduce_wrapper_grops(tmpbuf, resbuf, local_reduce_count, GPU_ON_NODE - 1, dt, op);
  if (MPI_SUCCESS != err)
  {
    goto err_hndl;
  }
  ...
  PICO_TAG_BEGIN("local_com/send_wait");
  err = MPI_Waitall(send_req_count, send_req, MPI_STATUSES_IGNORE);
  PICO_TAG_END("local_com/send_wait");
  PICO_TAG_END("local_com");
  ...
  while (mask < node_size)
  {
    ...
    PICO_TAG_BEGIN("global_com/block_recv");
    ...
    for (size_t block = recv_block_first; block <= recv_block_last; block++)
    {
      if (mask << 1 >= node_size)
      {
        ...
        err = MPI_Irecv((char *)recvbuf, recvcounts[inverse_remapping[block]], dt, partner * GPU_ON_NODE + local_rank, 0,
                        comm, &recv_req[recv_req_count]);
      }
      else
      {
        err = MPI_Irecv((char *)tmpbuf + displs[inverse_remapping[block]] * dtsize, recvcounts[inverse_remapping[block]], dt, partner * GPU_ON_NODE + local_rank, 0,
                        comm, &recv_req[recv_req_count]);
      }
      if (MPI_SUCCESS != err)
      {
    ...
    PICO_TAG_END("global_com/block_recv");

    PICO_TAG_BEGIN("global_com/block_send");
    for (size_t block = send_block_first; block <= send_block_last; block++)
    {
      err = MPI_Isend((char *)resbuf + displs[inverse_remapping[block]] * dtsize, recvcounts[inverse_remapping[block]], dt, partner * GPU_ON_NODE + local_rank, 0,
    ...
    PICO_TAG_END("global_com/block_send");

    int w_req = 0;
    for (size_t block = recv_block_first; block <= recv_block_last; block++)
    {
      PICO_TAG_BEGIN("global_com/recv_wait");
      err = MPI_Wait(&recv_req[w_req], MPI_STATUS_IGNORE);
      PICO_TAG_END("global_com/recv_wait");
    ...
    PICO_TAG_BEGIN("global_com/send_wait");
    err = MPI_Waitall(send_req_count, send_req, MPI_STATUSES_IGNORE);
    PICO_TAG_END("global_com/send_wait");
    if (MPI_SUCCESS != err)
    {
      goto err_hndl;
    }
